...
def numero_factura_existe(cursor, numeroFactura):
    ...
    query = "SELECT COUNT(*) FROM facturaventa WHERE numeroFactura = %s"
...
def guardar_factura(cursor, datos):
    ...
    query = """
        ...
        (numeroFactura, serie, pedidoId, clienteId, fechaEmision, fechaVencimiento, 
         estadoPago, baseImponible, ivaTotal, totalFactura, moneda, usuarioEmisorId) 
...
def validar_decimal(valor, nombre_campo):
    """Validar que un valor sea un número decimal válido"""
...
def validar_entero(valor, nombre_campo, obligatorio=False):
    ...
    if not valor or valor.strip() == '':
        if obligatorio:
            return None, f"{nombre_campo} es obligatorio"
        return None, None  # Es opcional y está vacío
    try:
    ...
    try:
        with open(file_path, 'r', encoding='utf-8') as file:
            ...
            for row in reader:
                ...
                # Validación: numeroFactura obligatorio
                ...
                # Validación: estadoPago obligatorio
                ...
                # Validación: usuarioEmisorId opcional
